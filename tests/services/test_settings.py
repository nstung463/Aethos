...
def test_load_managed_settings_merges_drop_ins_in_alphabetic_order(tmp_path: Path) -> None:
    ...
    service = SettingsService(config_home=tmp_path / "home-ethos", managed_settings_dir=managed)
...
def test_update_settings_for_source_writes_unified_schema(tmp_path: Path) -> None:
    ...
    data = json.loads((workspace / ".ethos" / "settings.json").read_text(encoding="utf-8"))
    assert data == {
        "permissions": {
            "mode": "accept_edits",
            "workingDirectories": ["src"],
...
def test_is_protected_ethos_path_matches_settings_and_skill_paths(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()

    assert is_protected_ethos_path(workspace, workspace / ".ethos" / "settings.json") is True
    assert is_protected_ethos_path(workspace, workspace / ".ethos" / "skills" / "demo" / "SKILL.md") is True
    assert is_protected_ethos_path(workspace, workspace / "src" / "app.py") is False
